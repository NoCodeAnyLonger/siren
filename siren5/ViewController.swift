...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        NetworkService.authErrorHandler = { _ in
            showAlert(title: "请先登录", message: nil, actions: UIAlertAction(title: "好", style: .default, handler: { [weak self] _ in
                self?.showLogin()
            }))
        }
...
extension ViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .unknown:
            showAlert(title: nil, message: "不支持的支付渠道\(info.paytype)", actions: UIAlertAction(title: "好", style: .cancel, handler: nil))
        }
    }
