    ...
    static let shared = PageSwitchManager(windowClosure: {
        if #available(iOS 13.0, *) {
            let activeWindowScene = UIApplication.shared.connectedScenes
                .filter { $0.activationState == .foregroundActive } // Focus on active scenes
                .compactMap { $0 as? UIWindowScene } // Cast to UIWindowScene
                .first // Get the first active scene
            
            // Return the key window from the active scene
            if let w = activeWindowScene?.keyWindow {
                return w
            }
            if let w = activeWindowScene?.windows.first {
                return w
            }
        } else {
